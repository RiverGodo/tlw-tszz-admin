    ...
    <Card shadow>
      ...
      <div class="list resource-list">
        <p class="list-info">
          ...
          <span>{{resourceSubjectCount}}</span>）&nbsp;份&nbsp;已选（
          <span>{{all_resource_id_arr.length}}</span>）</p>
        <Table ref="resourceList" :loading="resourceSubjectListLoading" :columns="resourceListDef" :data="resourceSubjectList" @on-selection-change="selectChange"></Table>
        <Row class="bottom-bar" type="flex" justify="space-between">
          ...
          <Page show-total show-sizer show-elevator :current="searchOption.page_index" :total="resourceSubjectCount" :page-size="searchOption.page_size" :page-size-opts="searchOption.page_size_opts" @on-page-size-change="pageSizeChange" @on-change="pageIndexChange">
          </Page>
          </Col>
        </Row>
      </div>
    </Card>
    <add-book-resource-modal :is-show="showAddResourceDialog" :resource-id="resource_id" @closeModal="showAddResourceDialog=false" @refreshList="getBookResourceSubjectList"></add-book-resource-modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      searchOption: state => state.bookResourceSubject.searchOption,
      resourceSubjectList: state => state.bookResourceSubject.resourceSubjectList,
      resourceSubjectListLoading: state => state.bookResourceSubject.resourceSubjectListLoading,
      resourceSubjectCount: state => state.bookResourceSubject.resourceSubjectCount
    }),
    ...mapGetters([
      "subjectListSelect",
      "cityListSelect",
  ...
  methods: {
    ...mapActions(["getBookResourceSubjectList"]),
    addLiveSource() {
      this.$router.push({
    ...
    init() {
      if (this.resourceSubjectList.length == 0) {
        this.getBookResourceSubjectList(this.resource_id_arr);
      }
    },
    handleSelectAll(status) {
      if (
        this.resource_id_arr[this.searchOption.page_index].length ==
        this.resourceSubjectList.length
      ) {
        this.$refs.resourceSubjectList.selectAll(false);
      } else {
        this.$refs.resourceSubjectList.selectAll(true);
      }
    },
    ...
    search(init) {
      if (init) {
        this.$store.commit("setBookResourceSubjectPageIndex", 1);
    ...
    pageIndexChange(page) {
      if (page != this.searchOption.page_index) {
        this.$store.commit("setBookResourceSubjectPageIndex", page);
        this.search();
      }
    },
    pageSizeChange(pageSize) {
      if (pageSize != this.searchOption.page_size) {
        this.$store.commit("setBookResourceSubjectPageSize", pageSize);
        this.search();
      }
    }
  }
