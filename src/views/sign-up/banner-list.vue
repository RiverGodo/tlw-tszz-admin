  ...
  <div class="page-content daily-english de-banner">
    ...
    <Card shadow>
      ...
      <div class="list banner-list">
        <Table ref="BannerList" :loading="signUpBannerListLoading" :columns="signUpBannerListDef" :data="signUpBannerList"></Table>
...
import { updateBanner, delBanner, orderByBanner } from "@/api/sign-up/banner";
export default {
  name: "banner-list",
  data() {
    return {
                ...
                on: {
                  "on-change": e => {
                    ...
                    for (const order of this.orderList) {
                      if (order.banner_id == params.row.subject_id) {
                        order.num = value;
                        break;
                      }
                    }
  ...
  computed: {
    ...mapState({
      ...
      signUpBannerList: state => state.signUpBanner.signUpBannerList,
      signUpBannerListLoading: state => state.signUpBanner.signUpBannerListLoading
    }),
    orderList: {
      get() {
        let arr = []
        for (const item of this.signUpBannerList) {
          arr.push({
  ...
  methods: {
    ...
    init() {
      ...
      this.signUpBannerList.length == 0 ? this.getBookBannerList() : "";
    },
    goAddBannerPage(bid) {
      if (bid > 0) {
        this.$router.push({
          path: "/SignUp/AddBanner" + "?bid=" + bid
        });
      } else {
        this.$router.push({
    ...
    setBannerState(banner) {
      ...
      // let newState = banner.state == 1 ? 0 : 1;
      this.$Modal.confirm({
        title: "提示信息",
        content: "确认" + str + "该广告吗？",
        onOk: res => {
    ...
    cancelChangeNum() {
      let arr = [];
      this.signUpBannerList.map(item => arr.push({ banner_id: item.banner_id, num: item.orderby }))
      this.orderList = arr
