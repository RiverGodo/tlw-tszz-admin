  ...
  <div class="page-content daily-english de-banner">
    ...
    <Card shadow>
      ...
      <div class="list banner-list">
        <Table ref="BannerList" :loading="bannerListLoading" :columns="bannerListDef" :data="bannerList"></Table>
...
import { updateBanner, delBanner, orderByBanner } from "@/api/daily-english/banner";
export default {
  name: "banner-list",
  data() {
    return {
  ...
  computed: {
    ...mapState({
      ...
      bannerListLoading: state => state.deBanner.bannerListLoading
    }),
    orderList: {
      get() {
        let arr = []
  ...
  methods: {
    ...
    init() {
      ...
      this.bannerList.length == 0 ? this.getBannerList() : "";
    },
    goAddBannerPage(bid) {
      if (bid > 0) {
        this.$router.push({
