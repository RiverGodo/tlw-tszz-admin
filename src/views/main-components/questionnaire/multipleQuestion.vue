    ...
    <div class="single-select">
        ...
        <div class="single-select-radio">
            <div class="single-select-radio-item" v-for="(item, index) in question.options">
                <input type="checkbox" :checked="filterOption(item.options_id)" disabled></input>
                <span>{{item.name}}</span>
            </div>
        </div>
...
export default {
  ...
  methods: {
    filterOption(v) {
      return this.question.answers.find(item => item == v) ? true : false;
    },
    editQuestion() {
      this.$emit("on-edit-question", { type: 1, index: this.SIndex });
    },
    delQuestion(v) {
