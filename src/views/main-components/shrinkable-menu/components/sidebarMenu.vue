...
<template>
    <Menu ref="sideMenu" :active-name="$route.name" :open-names="openNames" :theme="menuTheme" width="auto" @on-select="changeMenu">
        <template v-for="item in menuList">
            <!-- <MenuItem v-if="item.children.length == 1 && item.access == 2" :name="item.children[0].name" :key="item.path">
...
export default {
    ...
    computed: {
        ...mapState({
            currentActiveName: state => state.app.currentActiveName,
            currentOpenNames: state => state.app.currentOpenNames
        })
    },
    updated () {
        this.$nextTick(() => {
