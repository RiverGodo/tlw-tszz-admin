...
<template>
  <div class="page-content">
    ...
    <Card shadow>
      <div class="top-bar">
        ...
        <div class="actions" style="flex: 0 0 100px;">
          <Button type="primary" icon="android-add" @click="showAddBookDialogFun(-1)">添加课程</Button>
        </div>
      </div>
      <div class="list">
        <p class="list-info">
          <Icon type="ios-book"></Icon>当前课程&nbsp;（
    ...
    <!-- 推荐课程弹出框 -->
    <check-subject-recommend :is-show="showSubjectRecommendDialog" :recommendlist="recommendlist" :subcom_info="subcom_info" @closeModal="closeModal"></check-subject-recommend>
    <!-- 添加推荐课程弹出框 -->
    <add-recommend-modal :isShow="showAddRecommendDialog" :subcom_info="subcom_info" @closeModal="showAddRecommendDialog=false"></add-recommend-modal>
    <add-course-modal :is-show="showAddBookDialog" :book-id="book_id" @closeModal="showAddBookDialog=false" @refreshList="getBookList"></add-course-modal>
    <check-book-comment-modal :is-show="showCommentDialog" :commentlist="commentlist" :addcomment="book_info" :bookList="bookList" @closeModal="showCommentDialog=false"></check-book-comment-modal>
    <Modal class="common-dialog qrcode-modal" title="小程序二维码" v-model="showQrCode">
      <!-- <qrcode :value="qrCodeUrl" :options="{ size: 300,padding:10 }"></qrcode> -->
      <img :src="qrCodeUrl" alt="">
      <div>
        <Form :label-width="80">
          <FormItem label="设置来源">
              <Input v-model="qr_origin" clearable style="width:250px;margin-right:10px;"></Input>
              <Button @click="getQrCode(1)">生成新的二维码</Button>
          </FormItem>
        </Form>
      </div>
      <div slot="footer">
...
import {getBookCommendList}  from "@/api/book";
import { mapActions, mapState, mapGetters } from "vuex";
import addCourseModal from "../modal-components/add-course";
import addRecommendModal from "../modal-components/add-recommend";
import checkSubjectRecommend from "../modal-components/check-subject-recommend";
import checkBookCommentModal from "../modal-components/check-book-comment";
import {getMockBookCommentList} from "@/api/bookComment";
...
export default {
  ...
  components: {
    addCourseModal,
    qrcode,
    checkBookCommentModal,
    checkSubjectRecommend,
     addRecommendModal
  },
  data() {
    return {
      ...
      selectAuths: [],
      qr_origin: 'admin',   // 
      showAddRecommendDialog:false,
      showSubjectRecommendDialog:false,
      ...
      bookListDef: [
        ...
        },
        {
            ...
            render: (h, params) => {
                ...
                return h("Input", {
                    ...
                    on: {
                        "on-change": v => {
                            for (let e of this.bookList) {                                     
                            if (e.book_id == params.row.book_id) {
                                e.order_by = v.target.value;
                                break;
                            }
                            }
                        }
         ...
         }, */
        {
          ...
          render: (h, params) => {
            ...
            var items = ["下架", "上架", "兑换后可见"];
            var dropdownItems = items.map((item, idx) => {
        ...
        //           on: {
        //             "on-click": top => {
        //               this.setIsTop(params.row.book_id,top);
        //             }
        //           }
        //         },
        //         [
            ...
            let arr = [
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.showCommentDialogFun(params.row);
                    }
                  }
                },
                "查看评论"
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.showCheckSubjectRecommendDialogFun(params.row);
                    }
                  }
                },
                '查看推荐课程'
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.showAddRecommendDialogFun(params.row);
                    }
                  }
                },
                '添加推荐课程'
                ...
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      ...
                      this.book_id = params.row.book_id
                      this.getQrCode(params.row.book_id);
  ...
  computed: {
    ...
    ...mapState({
        CommendsearchOption: state => state.bookCommend.CommendsearchOption,
        bookCommendVerifyCodeStateList: state => state.bookCommend.bookCommendVerifyCodeStateList,
        bookCommendOnlineTestStateList: state => state.bookCommend.bookCommendOnlineTestStateList,
        CommendquestionnaireStateList: state => state.bookCommend.CommendquestionnaireStateList,
        bookCommendExStateList: state => state.bookCommend.bookCommendExStateList,
        bookCommendList: state => state.bookCommend.bookCommendList,
        bookCommendListLoading: state => state.bookCommend.bookCommendListLoading,
        bookCommendCount: state => state.bookCommend.bookCommendCount,
      }),
    ...mapGetters([
      "stateListSelect",
      "yearsListSelect",
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      if (from.name !== 'subject_resource_list') {
  ...
  methods: {
    ...
    //设置置顶功能,(book_id:图书id,top:置顶顺序值)
    setIsTop(book_id,top){
      switch(top) {
        case 0:
          this.setTop(book_id,top)
          break;
          case 1:
          this.cancelTop(book_id,top)
          break;
      }
    },
    init() {
      ...
      if (this.searchOption.type == 3) {
        ...
        this.$store.commit("Book_setSearchForm", { type: 3 });
        this.getBookList(this.book_id_arr);
      }
    },
    search(init) {
    ...
    closeModal(){
      this.showSubjectRecommendDialog=false;
    },
    //查看推荐课程
    showCheckSubjectRecommendDialogFun(params){
      this.subcom_info=params
      console.log(this.subcom_info.book_id)
      this.showSubjectRecommendDialog=true;
      getBookCommendList({
    ...
    //添加推荐课程
    showAddRecommendDialogFun(params){
      this.showAddRecommendDialog=true;
      this.subcom_info=params;
      this.getAddBookRecommendList();
    },
    ...
    getQrCode(_book_id){
      this.qrCodeUrl='';
      apiBook.getSmalProgramCode({
        book_id: this.book_id,
        origin: this.qr_origin
      }).then(res=>{
        if(res.res_code==1){
    ...
    //模拟评论
    showCommentDialogFun(params){
      console.log(params);
      this.book_info=params;
    ...
    delBook() {
      ...
      if (this.all_book_id_arr.length == 0) {
        ...
        // console.log(Boolean(this.selectAuths.size >=1 ));
        // console.log(Boolean(!userInfo.role_arr.includes(1)));
        // console.log(Boolean(!this.selectAuths.has(userInfo.name)));
        if(!((this.selectAuths.size >=1&&userInfo.role_arr.includes(1))||(this.selectAuths.size >=1&&this.selectAuths.has(userInfo.name)))){
          this.$Message.error('非创建者或管理员无法删除，请联系创建者或管理员')
         return
        }
        this.$Modal.confirm({
          title: "提示信息",
    ...
    setTop(book_id,top) {//置顶
      apiBook.setTop(book_id,top).then(res => {
        if (res.res_code == 1) {
    ...
    cancelTop(book_id, top) {//取消置顶
      ...
      apiBook.cancelTop(book_id,top).then(res => {
        if (res.res_code == 1) {
    ...
    cancelChangeNum(){
      let arr = [];
      this.bookList.map(item => arr.push({ subject_id: item.subject_id, num: item._index }))
      this.orderList = arr
      this.showChangeNum = false
    },
    changeNum(){
      this.newOrderList = JSON.parse(JSON.stringify(this.bookList));
        // console.log(this.recommendlist)
            this.changeNumLoading = true;
             apiBook.setBookOrderBy({
                order_by_arr:this.newOrderList
            }).then(res=>{
