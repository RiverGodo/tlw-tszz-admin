...
<template>
  <div class="page-content daily-english de-add-article">
    <Breadcrumb separator=">">
        ...
        <BreadcrumbItem to="/BookManage/ResourceList">资源管理</BreadcrumbItem>
        <BreadcrumbItem>添加直播</BreadcrumbItem>
    </Breadcrumb>
    <Card shadow>
      <div class="top-bar">
        <div class="options">
      ...
      <Form class="common-form add-article-form" ref="addArticleForm" :rules="ruleValidate" :model="addArticleForm" label-position="right"
        :label-width="120">
        ...
        <FormItem v-if="addArticleForm.type ==2 || addArticleForm.type ==3" :label="formLabel.resource" prop="online_resource_url">
          <div class="img-bar">
            <video-player class="video-bar" v-if="addArticleForm.online_resource_url !== ''" ref="videoPlayer" :options="playerOptions"
              :playsinline="true">
            </video-player>
            <div class="actions">
              <Row>
        ...
        <FormItem v-if="addArticleForm.type != 5" :label="formLabel.segment" prop="cover_img">
          <addSegment ref="segmentRef" :segmentData="addArticleForm.online_resource_ex"/>
        </FormItem>
        <FormItem v-if="addArticleForm.type == 5" label="推流地址" prop="push_flow_name">
          <Input @on-change="onKeyChange" v-model="push_flow_name" style="width: 200px" placeholder="请输入推流地址"></Input>
          <div v-if="showUrl">
            <p style="margin-top: 10px;">推流地址: rtmp://video-center-bj.alivecdn.com/<span style="color: #2d8cf0">{{push_flow_name}}</span>/01?vhost=live.xuege100.com</p>
            <p>播放地址: rtmp://live.xuege100.com/<span style="color: #2d8cf0">{{push_flow_name}}</span>/01</p>
          </div>
          <div v-if="addArticleForm.online_resource_stream_url && addArticleForm.online_resource_url && !showUrl">
            <p style="margin-top: 10px;">推流地址: {{addArticleForm.online_resource_stream_url}}</p>
            <p>播放地址: {{addArticleForm.online_resource_url}}</p>
          </div>
        </FormItem>
      ...
      <div style="text-align: center;">
        <Button type="primary" size="large" @click="goTransform()">转为录播</Button>
        <Button type="ghost" size="large" @click="goBack()">取消返回</Button>
        <Button type="primary" size="large" :loading="addArticleLoading" @click="AddArticle()">保存直播</Button>
      </div>
    </Card>
...
export default {
  ...
  methods: {
    ...
    onKeyChange() {
      if (this.push_flow_name) {
        this.showUrl = true;
      } else {
        this.showUrl = false;
      }

      this.addArticleForm.online_resource_stream_url = `rtmp://video-center-bj.alivecdn.com/${
        this.push_flow_name
      }/01?vhost=live.xuege100.com`;
      this.addArticleForm.play_url = `rtmp://live.xuege100.com/${
        this.push_flow_name
      }/01`;
    },
    goTransform() {
      ...
      if (this.$route.query.aid > 0) {
        ...
        getArticleDetail(this.$route.query.aid).then(res => {
          ...
          if (res.res_code == 1) {
            ...
            this.addArticleForm.start_time = new Date(res.msg.start_time);
            this.addArticleForm.end_time = new Date(res.msg.end_time);
            var rs = res.msg.online_resource_url.substr(
              res.msg.online_resource_url.indexOf("com/") + 4
            );
            this.push_flow_name = rs.substr(0, rs.indexOf("/"));
        ...
        this.addArticleForm.article_id = -1;
      }
      if (this.$route.query.cid > 0) {
        this.addArticleForm.category_id = this.$route.query.cid;
      }
      ...
      this.$refs["addArticleForm"].validate(valid => {
        if (valid) {
          ...
          if (this.addArticleForm.type == 5) {
            this.addArticleForm.online_resource_url = this.addArticleForm.play_url;
          }
          console.log(this.addArticleForm);
          this.addArticleLoading = true;
          if (this.addArticleForm.article_id > 0) {
